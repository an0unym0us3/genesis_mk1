...
class Map(pg.sprite.Sprite):
    def __init__(self, image, transp_image, attacked_image, low_health_image):
        ...
        transp_image = pg.transform.scale(transp_image,
                                          (transp_image.get_width() * bg_k, transp_image.get_height() * bg_k))
...
class Player(pg.sprite.Sprite):
    ...
    def movement_update(self, keys):
        ...
        self.image = pg.image.load(
            f'./Media/images/player/{"camouflage" if self.camouflage else "normal"}/{self.sprite_direction}player{self.leg_cycle[self.this_leg] if self.walking else ""}.png')
        self.walking = False
    ...
    def collide_update(self, object):
        ...
        if self.rect.colliderect(object.rect):
            ...
            abs(object.rect.x - (self.blit_pos[0] + self.w)) <= self.speed,
            ...
            global_pos = pg.Vector2(self.prev_pos.x + self.speed * self.offset[0],
                                    self.prev_pos.y + self.speed * self.offset[1])
...
class Minimap(pg.sprite.Sprite):
    ...
    def update(self):
        self.player_blit_pos = pg.Vector2(global_pos.x * mp_k / bg_k - self.player_w // 2 + 10,
                                          global_pos.y * mp_k / bg_k - self.player_h // 2 + 10)
...
class Object(pg.sprite.Sprite):
    ...
    def update(self):
        ...
        self.rect.x, self.rect.y = player.blit_pos.x + (self.left - global_pos[0]) + player.w / 2, player.blit_pos.y + (
                    self.top - global_pos[1]) + player.h / 2
        # pg.draw.rect(display, self.color, self.rect)
...
class Gun(pg.sprite.Sprite):
    def __init__(self, image, player, gun_space=25, gun_scale=1.25):
        ...
        for i in range(8):
            self.images.append(
                pg.transform.flip(pg.transform.rotate(self.image, (45 * i + ((3 <= i <= 5) * (4 - 2 * i) * 45))),
                                  (3 <= i <= 5), False))
        self.bullets = []
    ...
    def gun_tilt_update(self):
        ...
        self.blit_pos.x, self.blit_pos.y = (player_pos[0] + math.cos(self.angle) * self.gun_space), (
                    player_pos[1] - math.sin(self.angle) * self.gun_space / 2 + player.h / 10)
...
class Ghost(Object):
    def __init__(self, image=ghost_img, scale=0.5, speed=5, randomizer=50):
        ...
        for checkpoint in ghost_checkpoints:
            self.checkpoints.append((checkpoint[0] * bg_k + random.randrange(-randomizer, randomizer),
                                     checkpoint[1] * bg_k + random.randrange(-randomizer, randomizer)))
    ...
    def update_ghost(self):
        ...
        if abs(self.left - self.checkpoints[self.next_cp][0]) < 50 or abs(
                self.top - self.checkpoints[self.next_cp][1]) < 50:
...
class UI():
    ...
    def ui_blit(self, health, ammo, score, games, ingame_time):
        ...
        raw_rem_time = time_limit - ingame_time
        rem_time_m, rem_time_s = divmod(raw_rem_time, 60)
        rem_time = small_ui_font.render(f"Time left: {int(rem_time_m)}:{int(rem_time_s)}", False, (49, 51, 56))

        display.blit(rem_time, (window_c[0] - rem_time.get_width()//2, 10))
...
map = Map(image=true_bg_img, transp_image=transp_bg_img, attacked_image=attacked_bg_img,
          low_health_image=low_health_bg_img)
...
VICTORY_SCORE = 100
start_time = time.time() # data["start_time"]
# ingame_time =  data["ingame_time"]


while game_run:
    ingame_time = time.time() - start_time
    #min, sec = divmod(ingame_time, 60)
    #print(sec)
    if ingame_time >= time_limit:
        game_run = False
    ...
    text_surface = my_font.render(
        f"Score: {data}, Global: {global_pos}, Speed: {player.speed}, Mouse: {pg.mouse.get_pos()} Map:{map.w}  ", False,
        (200, 255, 200), (70, 100, 80))
    ...
    # player.draw()
    minimap.blit()
    ui.ui_blit(player.health, gun.ammo, data["score"], data["played_count"], ingame_time)
