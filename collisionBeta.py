...
class Player:
    def __init__(self, image_src, width=0, height=0):
        self.img = pg.image.load(image_src)
        self.w = self.img.get_width() if not width else width
        self.h = self.img.get_height() if not height else height
        self.pos = pg.Vector2(window_c[0] - self.img.get_width()//2, window_c[1] - self.img.get_height()//2)

    def colliding(self, pos_x, pos_y, this):
        delta_x = pos_x - this.pos.x
        delta_y = pos_y - this.pos.y

        impacts = []
        if delta_x <= this.w and delta_x >= 0:
            print('Left collision detected')
            impacts.append('left')
        elif -delta_x - self.w <= this.w and delta_x <= 0:
            print('Right collision detected')
            impacts.append('right')

        if delta_y <= this.h and delta_y >= 0:
            print('Top collision detected')
            impacts.append('top')
        elif -delta_y - self.h <= this.h and delta_y <= 0:
            print('Bottom collision detected')
            impacts.append('bottom')

        return impacts


class HitBox:
    def __init__(self, pos_x, pos_y, width=0, height=0, colour=(255,0,0)):
...
player_class = Player('./images/player/fplayer.png', 80, 120)
PLAYER_BLIT_CENTER = pg.Vector2(player_class.pos.x, player_class.pos.y)
print(PLAYER_BLIT_CENTER)
player_w, player_h = player_class.w, player_class.h
player_blit_pos = pg.Vector2(player_class.pos.x, player_class.pos.y)
obstacle = HitBox(900, 100, 80, 120)


mp_k = 0.4  # When translating core image to minimapa
...
mp_player_pos = pg.Vector2(mp_pos.x + mp_w // 2 - mplayer.get_width() // 2 + mp_spn[0],
                           mp_pos.y + mp_h // 2 - mplayer.get_height() // 2 + mp_spn[1])
...
while game_run:
    ...
    #
    #pg.draw.rect(display, (255, 0, 0), pg.Rect(200, 200, 60, 60))
    pg.draw.rect(display, obstacle.colour, pg.Rect(obstacle.pos.x, obstacle.pos.y, obstacle.w, obstacle.h))

    collisions = player_class.colliding(player_blit_pos.x, player_blit_pos.y, obstacle)
    if collisions:
        print('Collided at', collisions)

    # Draw the player on top of the minimap.
    # Needs a bit more readjusting, but now the minimap player works based on calculation and doesn't need it's own speed
    '''
    ...
    text_surface = my_font.render(
        f"Mouse_Pos: {pg.mouse.get_pos()} Player_Pos:{map_pos} Blit Pos:{player_blit_pos} Speed:{speed} Bg:{bg_w, bg_h} Map:{mp_player_pos, (map_pos.y - window_h) // (bg_k / mp_k)}",
        False, (200, 255, 200), (70, 100, 80))
    display.blit(text_surface, (0, window_h - 24))
    flag_debug = my_font.render(
        f"player_blit_pos Flag:{player_blit_pos.x <= PLAYER_BLIT_CENTER.x}            map_pos Flag:{map_pos.x >= 0}",
        False, (200, 255, 200), (70, 100, 80))
